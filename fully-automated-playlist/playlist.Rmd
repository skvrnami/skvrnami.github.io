...
WEEKDAY <- lubridate::wday(as.Date(Sys.Date()), 
                           week_start = 1, 
                           locale = "cs_CZ.UTF-8")
...
NOTE <- glue::glue("Nejčastěji přehrávané písně na Radiu Wave mezi {START_DATE} a {END_DATE}")
...
top_tracks_clustered %>%
  ...
  cols_align("left") %>%
  tab_header(title = "Fully Automated Playlist", 
             subtitle = NOTE) %>%
